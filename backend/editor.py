...
class Editor:
    ...
    def edit_video_overlays(self, video_path, output_path, overlay_params=None, text_params=None, progress_callback=None):
        try:
            ...
            if overlay_params and os.path.exists(overlay_params['image_path']):
                ...
                opacity = max(0.0, min(1.0, float(overlay_params.get('opacity', 1.0))))
                image = ffmpeg.filter(image, 'scale', f"iw*{scale}", f"ih*{scale}")
                if opacity < 1.0:
                    image = ffmpeg.filter(image, 'format', 'rgba')
                    image = ffmpeg.filter(image, 'colorchannelmixer', aa=opacity)
            ...
            if text_params and text_params.get('text'):
                ...
                opacity = max(0.0, min(1.0, float(text_params.get('opacity', 1.0))))
                font_name = text_params.get('font', 'Arial')
                video_stream = ffmpeg.drawtext(
                    video_stream,
                    text=text,
                    fontfile='C:\\Windows\\Fonts\\arial.ttf' if font_name == 'Arial' else font_name,
                    fontsize=font_size,
                    fontcolor=color,
                    alpha=opacity,
                    x=x,
    ...
    def edit_video_image_overlay(self, video_path, output_path, image_params, progress_callback=None):
        try:
            image_path, pos_x, pos_y, width, opacity = image_params
            video = VideoFileClip(video_path)
            ...
            image_clip = image_clip.set_opacity(max(0.0, min(1.0, opacity)))
    ...
    def edit_video_color_grading(self, video_path, output_path, color_params, progress_callback=None):
        try:
