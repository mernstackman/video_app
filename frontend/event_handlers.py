...
class EventHandler:
    ...
    def select_video(self):
        ...
        if file_path:
            ...
            self.ui.update_preview()  # Update preview after selecting a video
    ...
    def select_overlay_image(self):
        ...
        if file_path:
            self.ui.image_controls_widget.overlay_image_path.set(file_path)
            self.ui.update_status(f"Selected overlay image: {os.path.basename(file_path)}")
            self.ui.update_preview()  # Update preview after selecting an overlay image
    ...
    def update_video_info(self):
        ...
        if not video_path:
            ...
            # Reset crop fields when no video is selected
            if hasattr(self.ui, 'crop_controls_widget'):
                self.ui.crop_controls_widget.crop_x2.set("0")
                self.ui.crop_controls_widget.crop_y2.set("0")
            return
        try:
            ...
            if video_stream:
                ...
                # Update crop fields with video dimensions
                if hasattr(self.ui, 'crop_controls_widget'):
                    self.ui.crop_controls_widget.crop_x2.set(str(width))
                    self.ui.crop_controls_widget.crop_y2.set(str(height))
            else:
                self.ui.video_info.set("No video stream found.")
                # Reset crop fields if no video stream
                if hasattr(self.ui, 'crop_controls_widget'):
                    self.ui.crop_controls_widget.crop_x2.set("0")
                    self.ui.crop_controls_widget.crop_y2.set("0")
    ...
    def download_from_url(self):
        ...
        try:
            ...
            if success:
                ...
                self.update_video_info()
                self.ui.update_preview()  # Update preview after downloading
            else:
    ...
    def apply_overlays(self):
        ...
        try:
            overlay_params = {
                'image_path': self.ui.image_controls_widget.overlay_image_path.get(),
                'x': int(self.ui.image_controls_widget.overlay_x.get() or 0),
                'y': int(self.ui.image_controls_widget.overlay_y.get() or 0),
                'scale': float(self.ui.image_controls_widget.overlay_scale.get() or 1.0),
                'opacity': float(self.ui.image_controls_widget.overlay_opacity.get() or 1.0)
            } if self.ui.image_controls_widget.overlay_image_path.get() else None
            text_params = {
                'text': self.ui.text_controls_widget.overlay_text.get(),
                'font_size': int(self.ui.text_controls_widget.text_font_size.get() or 24),
                'color': self.ui.text_controls_widget.text_color.get() or 'white',
                'x': int(self.ui.text_controls_widget.text_x.get() or 10),
                'y': int(self.ui.text_controls_widget.text_y.get() or 10),
                'font': self.ui.text_controls_widget.text_font.get() or 'Arial',
                'opacity': float(self.ui.text_controls_widget.text_opacity.get() or 1.0)
            } if self.ui.text_controls_widget.overlay_text.get() else None
